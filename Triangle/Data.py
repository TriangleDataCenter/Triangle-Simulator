...
def SliceDataDictionary(data_dict, time, time_start, time_end, buffer_time=80, channels=['A2', 'E2', 'T2']):
    ...
    for chan in data_dict.keys():
        ...
        if chan in channels:
            sliced_data_dict[chan][:indmin] = 0.
            sliced_data_dict[chan][indmax:] = 0.
...
def AddDicts(dict_list, coef_list=1.):
    """
...
def MultiplyDicts(dict1, dict2, coef=1.):
...
def uniform_lagrange_timeshift(data, shifts, order=31):
    ...
    for i in range(Ndata):
        ...
        G = np.zeros(p - 1)
        for j in range(1, p):
            A *= (1. + eps[i] / j) * (1. + (1. - eps[i]) / j)
            E[j - 1] = (-1.) ** j * np.math.factorial(p - 1) * np.math.factorial(p) \
                    / np.math.factorial(p - 1 - j) / np.math.factorial(p + j)
            F[j - 1] = j + eps[i]
            G[j - 1] = j + 1. - eps[i]
