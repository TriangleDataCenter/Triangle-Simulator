...
class GeneralNoise():
    ...
    def _TimeDomainNoiseGeneration(self, fsample, size):
        sigma0 = np.sqrt(fsample / 2.)
        n0 = np.random.normal(0, sigma0, size)

        n0f = fft(n0)
        f_arr = fftfreq(size, d = 1. / fsample)
    ...
    def FrequencyDomainNoiseGeneration(self, freqs):
        ...
        noise_Re = np.random.randn(size) * np.sqrt(psd / 4. / df)
        noise_Im = np.random.randn(size) * np.sqrt(psd / 4. / df)
        noise_total = noise_Re + 1.j * noise_Im
        return noise_total
...
class InstrumentalPSDs():
    ...
    def PSD_ACC(self, f, sacc = SACC_nominal): 
        if self.unit == 'ffd':
            u = TWOPI * f * self.L / C
            return (sacc * self.L / u / C ** 2) ** 2 * (1. + (0.4e-3 / f) ** 2) * (1. + (f / 8e-3) ** 4)
        elif self.unit == 'frequency':
            u = TWOPI * f * self.L / C
            return (sacc * self.L / u / C ** 2) ** 2 * (1. + (0.4e-3 / f) ** 2) * (1. + (f / 8e-3) ** 4) * F_LASER ** 2


    def PSD_RO(self, f, sro = SOMS_nominal): 
        if self.unit == 'ffd':
            u = TWOPI * f * self.L / C
            return (u * sro / self.L) ** 2 * (1. + (2e-3 / f) ** 4)
        elif self.unit == 'frequency':
            u = TWOPI * f * self.L / C
            return (u * sro / self.L) ** 2 * (1. + (2e-3 / f) ** 4) * F_LASER ** 2
    ...
    def PSD_BL(self, f, sbl = SBL_nominal): 
        if self.unit == 'ffd':
            return (TWOPI * f * sbl / C) ** 2 * (1. + (2e-3 / f) ** 4)
        elif self.unit == 'frequency':
            return (TWOPI * f * sbl / C) ** 2 * (1. + (2e-3 / f) ** 4) * F_LASER ** 2
...
class TDIPSDs():
    ...
    def PSD_X(self, f): # self.sa, self.so are the asd of acc and opt noise, WTN and wang and Vallisneri
        ...
        Sa = self.PSD_Sa(f)
        So = self.PSD_So(f)
        return Sa * (8. * (np.sin(2. * u)) ** 2 + 32. * (np.sin(u)) ** 2) + 16. * So * (np.sin(u)) ** 2
    ...
    def PSD_X2_unequal(self, f, arms):
        ...
        u12131 = u121 + u131
        s121 = np.sin(u121 / 2.) ** 2
        s131 = np.sin(u131 / 2.) ** 2
        c121 = np.cos(u121 / 2.) ** 2
        c131 = np.cos(u131 / 2.) ** 2
        s12131 = np.sin(u12131 / 2.) ** 2
        OMS_term = 32. * So * s12131 * (s121 + s131)
        ACC_term = 64. * Sa * s12131 * (s121 * (c131 + 1) + s131 * (c121 + 1))
    ...
    def PSD_Y2_unequal(self, f, arms):
        ...
        u23212 = u232 + u212
        s232 = np.sin(u232 / 2.) ** 2
        s212 = np.sin(u212 / 2.) ** 2
        c232 = np.cos(u232 / 2.) ** 2
        c212 = np.cos(u212 / 2.) ** 2
        s23212 = np.sin(u23212 / 2.) ** 2
        OMS_term = 32. * So * s23212 * (s232 + s212)
        ACC_term = 64. * Sa * s23212 * (s232 * (c212 + 1) + s212 * (c232 + 1))
        return OMS_term + ACC_term
    ...
    def PSD_Z2_unequal(self, f, arms):
        ...
        u31323 = u313 + u323
        s313 = np.sin(u313 / 2.) ** 2
        s323 = np.sin(u323 / 2.) ** 2
        c313 = np.cos(u313 / 2.) ** 2
        c323 = np.cos(u323 / 2.) ** 2
        s31323 = np.sin(u31323 / 2.) ** 2
        OMS_term = 32. * So * s31323 * (s313 + s323)
        ACC_term = 64. * Sa * s31323 * (s313 * (c323 + 1) + s323 * (c313 + 1))
        return OMS_term + ACC_term
    ...
    def PSD_X2Y2star_unequal(self, f, arms):
        ...
        u212 = TWOPI * f * (arms['21'] + arms['12'])
        u23212 = u232 + u212
        D21 = np.exp(-1.j * TWOPI * f * arms['21'])
        D12 = np.exp(-1.j * TWOPI * f * arms['12'])
        D131 = np.exp(-1.j * u131)
        D12131 = np.exp(-1.j * u12131)
        D232 = np.exp(-1.j * u232)
        D23212 = np.exp(-1.j * u23212)
        delay_factor = (1. - D131) * (1. - D12131) * (1. - D232.conjugate()) * (1. - D23212.conjugate()) * (D21.conjugate() + D12)
        OMS_term = -So 
        ACC_term = -4. * Sa 
    ...
    def PSD_Y2Z2star_unequal(self, f, arms):
        ...
        u323 = TWOPI * f * (arms['32'] + arms['23'])
        u31323 = u313 + u323
        D32 = np.exp(-1.j * TWOPI * f * arms['32'])
        D23 = np.exp(-1.j * TWOPI * f * arms['23'])
        D212 = np.exp(-1.j * u212)
        D23212 = np.exp(-1.j * u23212)
        D313 = np.exp(-1.j * u313)
        D31323 = np.exp(-1.j * u31323)
        delay_factor = (1. - D212) * (1. - D23212) * (1. - D313.conjugate()) * (1. - D31323.conjugate()) * (D32.conjugate() + D23)
        OMS_term = -So 
        ACC_term = -4. * Sa 
        return (OMS_term + ACC_term) * delay_factor
    ...
    def PSD_Z2X2star_unequal(self, f, arms):
        ...
        u131 = TWOPI * f * (arms['13'] + arms['31'])
        u12131 = u121 + u131
        D13 = np.exp(-1.j * TWOPI * f * arms['13'])
        D31 = np.exp(-1.j * TWOPI * f * arms['31'])
        D323 = np.exp(-1.j * u323)
        D31323 = np.exp(-1.j * u31323)
        D121 = np.exp(-1.j * u121)
        D12131 = np.exp(-1.j * u12131)
        delay_factor = (1. - D323) * (1. - D31323) * (1. - D121.conjugate()) * (1. - D12131.conjugate()) * (D13.conjugate() + D31)
        OMS_term = -So 
        ACC_term = -4. * Sa 
        return (OMS_term + ACC_term) * delay_factor
    ...
    def PSD_A2(self, f):
        u = TWOPI * f * self.L / C
        return np.maximum(4. * (np.sin(2. * u)) ** 2 * self.PSD_A(f), 1e-50) 
    ...
    def PSD_E2_unequal(self, f, arms):
        return (self.PSD_X2_unequal(f, arms) + 4. * self.PSD_Y2_unequal(f, arms) + self.PSD_Z2_unequal(f, arms) \
            -4. * np.real(self.PSD_X2Y2star_unequal(f, arms)) + 2. * np.real(self.PSD_Z2X2star_unequal(f, arms)) \
            -4. * np.real(self.PSD_Y2Z2star_unequal(f, arms))) / 6.
        
    def PSD_T2_unequal(self, f, arms):
        return (self.PSD_X2_unequal(f, arms) + self.PSD_Y2_unequal(f, arms) + self.PSD_Z2_unequal(f, arms) \
            +2. * np.real(self.PSD_X2Y2star_unequal(f, arms)) + 2. * np.real(self.PSD_Z2X2star_unequal(f, arms)) \
            +2. * np.real(self.PSD_Y2Z2star_unequal(f, arms))) / 3.
    ...
    def PSD_X2_unequal_different_amps(self, f, arms, A_OMS, A_ACC):
        ...
        c131 = np.cos(u131 / 2.) ** 2
        s12131 = np.sin(u12131 / 2.) ** 2
        OMS_term = 16. * s12131 * (s131 * (A_OMS['12'] ** 2 + A_OMS['21'] ** 2) + s121 * (A_OMS['13'] ** 2 + A_OMS['31'] ** 2))
        ACC_term = 64. * s12131 * (s131 * (c121 * A_ACC['12'] ** 2 + A_ACC['21'] ** 2) + s121 * (c131 * A_ACC['13'] ** 2 + A_ACC['31'] ** 2))
    ...
    def PSD_Y2_unequal_different_amps(self, f, arms, A_OMS, A_ACC):
        ...
        c212 = np.cos(u212 / 2.) ** 2
        s23212 = np.sin(u23212 / 2.) ** 2
        OMS_term = 16. * s23212 * (s212 * (A_OMS['23'] ** 2 + A_OMS['32'] ** 2) + s232 * (A_OMS['21'] ** 2 + A_OMS['12'] ** 2))
        ACC_term = 64. * s23212 * (s212 * (c232 * A_ACC['23'] ** 2 + A_ACC['32'] ** 2) + s232 * (c212 * A_ACC['21'] ** 2 + A_ACC['12'] ** 2))
        return OMS_term * So + ACC_term * Sa 
    ...
    def PSD_Z2_unequal_different_amps(self, f, arms, A_OMS, A_ACC):
        ...
        c323 = np.cos(u323 / 2.) ** 2
        s31323 = np.sin(u31323 / 2.) ** 2
        OMS_term = 16. * s31323 * (s323 * (A_OMS['31'] ** 2 + A_OMS['13'] ** 2) + s313 * (A_OMS['32'] ** 2 + A_OMS['23'] ** 2))
        ACC_term = 64. * s31323 * (s323 * (c313 * A_ACC['31'] ** 2 + A_ACC['13'] ** 2) + s313 * (c323 * A_ACC['32'] ** 2 + A_ACC['23'] ** 2))
        return OMS_term * So + ACC_term * Sa 
    ...
    def PSD_X2Y2star_unequal_different_amps(self, f, arms, A_OMS, A_ACC):
        ...
        D232 = np.exp(-1.j * u232)
        D23212 = np.exp(-1.j * u23212)
        OMS_term = -1. * (1. - D131) * (1. - D12131) * np.conjugate((1. - D232) * (1. - D23212)) * (np.conjugate(D21) * A_OMS['12'] ** 2 + D12 * A_OMS['21'] ** 2)
        ACC_term = -2. * (1. - D131) * (1. - D12131) * np.conjugate((1. - D232) * (1. - D23212)) * ((np.conjugate(D21) + D12) * (A_ACC['12'] ** 2 + A_ACC['21'] ** 2))
        return OMS_term * So + ACC_term * Sa 
    ...
    def PSD_Y2Z2star_unequal_different_amps(self, f, arms, A_OMS, A_ACC):
        ...
        D313 = np.exp(-1.j * u313)
        D31323 = np.exp(-1.j * u31323)
        OMS_term = -1. * (1. - D212) * (1. - D23212) * np.conjugate((1. - D313) * (1. - D31323)) * (np.conjugate(D32) * A_OMS['23'] ** 2 + D23 * A_OMS['32'] ** 2)
        ACC_term = -2. * (1. - D212) * (1. - D23212) * np.conjugate((1. - D313) * (1. - D31323)) * ((np.conjugate(D32) + D23) * (A_ACC['23'] ** 2 + A_ACC['32'] ** 2))
        return OMS_term * So + ACC_term * Sa 
    ...
    def PSD_Z2X2star_unequal_different_amps(self, f, arms, A_OMS, A_ACC):
        ...
        D121 = np.exp(-1.j * u121)
        D12131 = np.exp(-1.j * u12131)
        OMS_term = -1. * (1. - D323) * (1. - D31323) * np.conjugate((1. - D121) * (1. - D12131)) * (np.conjugate(D13) * A_OMS['31'] ** 2 + D31 * A_OMS['13'] ** 2)
        ACC_term = -2. * (1. - D323) * (1. - D31323) * np.conjugate((1. - D121) * (1. - D12131)) * ((np.conjugate(D13) + D31) * (A_ACC['31'] ** 2 + A_ACC['13'] ** 2))
        return OMS_term * So + ACC_term * Sa 
    ...
    def PSD_E2_unequal_different_amps(self, f, arms, A_OMS, A_ACC):
        return (self.PSD_X2_unequal_different_amps(f, arms, A_OMS, A_ACC) + 4. * self.PSD_Y2_unequal_different_amps(f, arms, A_OMS, A_ACC) + self.PSD_Z2_unequal_different_amps(f, arms, A_OMS, A_ACC) \
            -4. * np.real(self.PSD_X2Y2star_unequal_different_amps(f, arms, A_OMS, A_ACC)) + 2. * np.real(self.PSD_Z2X2star_unequal_different_amps(f, arms, A_OMS, A_ACC)) \
            -4. * np.real(self.PSD_Y2Z2star_unequal_different_amps(f, arms, A_OMS, A_ACC))) / 6.
        
    def PSD_T2_unequal_different_amps(self, f, arms, A_OMS, A_ACC):
        return (self.PSD_X2_unequal_different_amps(f, arms, A_OMS, A_ACC) + self.PSD_Y2_unequal_different_amps(f, arms, A_OMS, A_ACC) + self.PSD_Z2_unequal_different_amps(f, arms, A_OMS, A_ACC) \
            +2. * np.real(self.PSD_X2Y2star_unequal_different_amps(f, arms, A_OMS, A_ACC)) + 2. * np.real(self.PSD_Z2X2star_unequal_different_amps(f, arms, A_OMS, A_ACC)) \
            +2. * np.real(self.PSD_Y2Z2star_unequal_different_amps(f, arms, A_OMS, A_ACC))) / 3.
    ...
    def PSD_T2(self, f): 
        u = TWOPI * f * self.L / C
        return 4. * (np.sin(2. * u)) ** 2 * self.PSD_T(f) 
    
    def PSD_alpha(self, f): # wang, Vallis
        u = TWOPI * f * self.L / C
        Sa = self.PSD_Sa(f)
        So = self.PSD_So(f)
        return 8. * Sa * ((np.sin(1.5 * u)) ** 2 + 2. * (np.sin(0.5 * u)) ** 2) \
                + 6. * So
                
    def PSD_alpha2(self, f):
        u = TWOPI * f * self.L / C
        return self.PSD_alpha(f) * 4. * np.sin(1.5 * u) ** 2
    ...
    def PSD_alpha2_unequal(self, f, arms):
        ...
        u132 = u13 + u23 
        u1231 = u123 + u13 
        OMS_term = 24. * So * np.sin(u1231 / 2.) ** 2 
        ACC_term = 32. * Sa * np.sin(u1231 / 2.) ** 2 * (np.sin(u1231 / 2.) ** 2 + np.sin((u132 - u12) / 2.) ** 2 + np.sin((u123 - u13) / 2.) ** 2)
        return OMS_term + ACC_term
    ...
    def PSD_beta2_unequal(self, f, arms):
        ...
        u213 = u21 + u31 
        u2312 = u231 + u21  
        OMS_term = 24. * So * np.sin(u2312 / 2.) ** 2 
        ACC_term = 32. * Sa * np.sin(u2312 / 2.) ** 2 * (np.sin(u2312 / 2.) ** 2 + np.sin((u213 - u23) / 2.) ** 2 + np.sin((u231 - u21) / 2.) ** 2)
        return OMS_term + ACC_term
    ...
    def PSD_gamma2_unequal(self, f, arms):
        ...
        u321 = u32 + u12 
        u3123 = u312 + u32    
        OMS_term = 24. * So * np.sin(u3123 / 2.) ** 2 
        ACC_term = 32. * Sa * np.sin(u3123 / 2.) ** 2 * (np.sin(u3123 / 2.) ** 2 + np.sin((u321 - u31) / 2.) ** 2 + np.sin((u312 - u32) / 2.) ** 2)
        return OMS_term + ACC_term
    ...
    def PSD_alpha2gamma2star_unequal(self, f, arms):
        ...
        u132 = u13 + u23 
        u1231 = u123 + u13 
        prefactor = 4. * np.sin(u1231 / 2.) ** 2
        OMS_term = 2. * So * (2. * np.cos(u13) + np.cos(u123))
        ACC_term = 4. * Sa * (np.cos(u13) - np.cos(u23 - u12))
        return prefactor * (OMS_term + ACC_term)
    
    def PSD_P22_unequal(self, f, arms):
        return 0.5 * (self.PSD_alpha2_unequal(f, arms) + self.PSD_gamma2_unequal(f, arms) + 2. * self.PSD_alpha2gamma2star_unequal(f, arms))
...
def ResidualClockNoise(freqs, a, b, idx=0, channel='X2'):
    ...
    if channel == 'X2':
        ...
        term2 = (a['21'][idx]) ** 2
        term3 = (a['31'][idx]) ** 2
        term4 = -4. * b['12'][idx] * (a['12'][idx] - a['13'][idx] - b['12'][idx]) * (np.sin(u)) ** 2
    if channel == 'Y2':
        term1 = (a['23'][idx] - a['21'][idx]) ** 2
        term2 = (a['32'][idx]) ** 2
        term3 = (a['12'][idx]) ** 2
        term4 = -4. * b['23'][idx] * (a['23'][idx] - a['21'][idx] - b['23'][idx]) * (np.sin(u)) ** 2
    if channel == 'Z2':
        term1 = (a['31'][idx] - a['32'][idx]) ** 2
        term2 = (a['13'][idx]) ** 2
        term3 = (a['23'][idx]) ** 2
        term4 = -4. * b['31'][idx] * (a['31'][idx] - a['32'][idx] - b['31'][idx]) * (np.sin(u)) ** 2
    return 16. * (np.sin(2. * u)) ** 2 * (np.sin(u)) ** 2 * (term1 + term2 + term3 + term4) * S_q
...
def PSD_laser_residual_from_bias(f, delay_bias, channel='X2'):
    ...
    if channel == 'X2':
        ...
        B13 = delay_bias['13']
        B31 = delay_bias['31']
        return 16. * np.sin(u) ** 2 * np.sin(2. * u) ** 2 * omega ** 2 * (B12 ** 2 + B21 ** 2 + B13 ** 2 + B31 ** 2) * psd 
    elif channel == 'Y2':
        B23 = delay_bias['23']
        B32 = delay_bias['32']
        B21 = delay_bias['21']
        B12 = delay_bias['12']
        return 16. * np.sin(u) ** 2 * np.sin(2. * u) ** 2 * omega ** 2 * (B23 ** 2 + B32 ** 2 + B21 ** 2 + B12 ** 2) * psd 
    elif channel == 'Z2':
        B31 = delay_bias['31']
        B13 = delay_bias['13']
        B32 = delay_bias['32']
        B23 = delay_bias['23']
        return 16. * np.sin(u) ** 2 * np.sin(2. * u) ** 2 * omega ** 2 * (B31 ** 2 + B13 ** 2 + B32 ** 2 + B23 ** 2) * psd 
    elif channel == 'A2':
        B12 = delay_bias['12']
        B21 = delay_bias['21']
        B32 = delay_bias['32']
        ...
        B13 = delay_bias['13']
        B31 = delay_bias['31']
        return 8. * np.sin(u) ** 2 * np.sin(2. * u) ** 2 * omega ** 2 \
            * (B12 ** 2 + B21 ** 2 + B32 ** 2 + B23 ** 2 + 4. * np.cos(u/2.) ** 2 * (B31 ** 2 + B13 ** 2)) * psd 
    elif channel == 'E2':
        B12 = delay_bias['12']
        B21 = delay_bias['21']
        B32 = delay_bias['32']
        ...
        B13 = delay_bias['13']
        B31 = delay_bias['31']
        return 8. / 3. * np.sin(u) ** 2 * np.sin(2. * u) ** 2 * omega ** 2 \
            * ((5. + 4. * np.cos(u)) * (B12 ** 2 + B21 ** 2 + B32 ** 2 + B23 ** 2) + 4. * np.sin(u/2.) ** 2 * (B31 ** 2 + B13 ** 2)) * psd 
    elif channel == 'T2':
        B12 = delay_bias['12']
        B21 = delay_bias['21']
        B32 = delay_bias['32']
        ...
        B13 = delay_bias['13']
        B31 = delay_bias['31']
        return 64. / 3. * np.sin(u) ** 2 * np.sin(2. * u) ** 2 * omega ** 2 * np.sin(u / 2.) ** 2 \
            * (B12 ** 2 + B21 ** 2 + B32 ** 2 + B23 ** 2 + B31 ** 2 + B13 ** 2) * psd     
    else:
...
class TDIPSDFromPString():
    ...
    Y2_strings = {
        "23": [(-1., []), (1., ["21", "12"]), (1., ["21", "12", "23", "32"]), (-1., ["23", "32", "21", "12", "21", "12"])], 
        "31": [], 
        "12": [(1., ["21"]), (-1., ["23", "32", "21"]), (-1., ["23", "32", "21", "12", "21"]), (1., ["21", "12", "23", "32", "23", "32", "21"])], 
        "32": [(-1., ["23"]), (1., ["21", "12", "23"]), (1., ["21", "12", "23", "32", "23"]), (-1., ["23", "32", "21", "12", "21", "12", "23"])], 
        "13": [], 
        "21": [(1., []), (-1., ["23", "32"]), (-1., ["23", "32", "21", "12"]), (1., ["21", "12", "23", "32", "23", "32"])], 
    }
    Z2_strings = {
        "31": [(-1., []), (1., ["32", "23"]), (1., ["32", "23", "31", "13"]), (-1., ["31", "13", "32", "23", "32", "23"])], 
        "12": [], 
        "23": [(1., ["32"]), (-1., ["31", "13", "32"]), (-1., ["31", "13", "32", "23", "32"]), (1., ["32", "23", "31", "13", "31", "13", "32"])], 
        "13": [(-1., ["31"]), (1., ["32", "23", "31"]), (1., ["32", "23", "31", "13", "31"]), (-1., ["31", "13", "32", "23", "32", "23", "31"])], 
        "21": [], 
        "32": [(1., []), (-1., ["31", "13"]), (-1., ["31", "13", "32", "23"]), (1., ["32", "23", "31", "13", "31", "13"])], 
    }
    C_12_3_strings = {
        "13": [(1., ["32", "23", "-31"]), (-1., ["31", "-12", "23", "-31"])], 
        "21": [(-1., ["31", "-12"]), (1., ["31", "-12", "23", "-31", "12"])], 
        "12": [(1., ["31", "-12", "23", "-31"]), (-1., ["32", "23", "-31"])],
        "23": [(-1., ["32"]), (1., ["31", "-12"])],
        "31": [(1., []), (-1., ["32", "23", "-31", "12", "-23"])], 
        "32": [(-1., []), (1., ["32", "23", "-31", "12", "-23"])],
    }
    C_14_3_strings = {
        "12": [(1., ["23", "-31"]), (-1., ["21", "12", "23", "-31"]), (1., ["23", "-31", "12", "-23", "31"]), (-1., ["21"])], 
        "13": [(-1., ["23", "-31"]), (1., ["21", "12", "23", "-31"])], 
        "32": [(-1., ["23", "-31", "12", "-23"]), (1., ["21", "12", "23", "-31", "12", "-23"])], 
        "21": [(-1., []), (1., ["23", "-31", "12", "-23", "31", "12"])], 
        "31": [(1., ["23", "-31", "12", "-23"]), (-1., ["21", "12", "23", "-31", "12", "-23"])], 
        "23": [(1., []), (-1., ["21", "12"])], 
    }
    C_16_28_strings = {
        "12": [(1., ["-12", "23", "-31"]), (-1., ["-13", "-31", "12", "23", "-31"]), (1., ["-12", "23", "-31", "12", "-23", "-31"]), (-1., ["-13", "-31"])],
        "32": [(-1., ["-12", "23", "-31", "12", "-23"]), (1., ["-13", "-31", "12", "23", "-31", "12", "-23"])],
        "23": [(1., ["-12"]), (-1., ["-13", "-31", "12"])],
        "13": [(1., ["-13", "-31", "12", "23", "-31"]), (-1., ["-12", "23", "-31"]), (-1., ["-12", "23", "-31", "12", "-23", "-31"]), (1., ["-13", "-31"])],
        "31": [(1., ["-13"]), (-1., ["-13", "-31", "12", "23", "-31", "12", "-23"])],
        "21": [(-1., ["-12"]), (1., ["-12", "23", "-31", "12", "-23", "-31", "12"])],
    }
