...
class TDI():
    ...
    def __init__(self, measurements, delays, fsample, order=31, delay_order=5):
        ...
        if not 'dpl' in self.measurements.keys():
    ...
    def CalculateXi(self, doppler=True, pool=None):
        ...
        """
        if doppler:
            d_args = dict(fsample=self.fsample, delay=self.delays, doppler=self.measurements['dpl'], order=self.order)
        else:
    ...
    def CalculateEta(self, doppler=True):
        ...
        for k in left_MOSA_labels:
            ...
            if doppler:
                delayed_ref *= (1. - self.measurements['dpl'][k])
    ...
    def CalculatePath(self, Path_string, eta_dict, doppler=True):
        ...
        for i in range(N_measurement):
            ...
            for j in range(N_delay):
                ...
                total_delay_time = total_delay_time - timeshift(self.delays[delay_idx], total_delay_time * self.fsample, order=self.delay_order) # delays can be calculated with low-order interpolation
            if doppler == True:
                total_delay_doppler = np.gradient(total_delay_time, 1./self.fsample)
                tdi += (1. + total_delay_doppler) * timeshift(eta_dict[delay_array[i]['measurement']], total_delay_time * self.fsample, order=self.order)
            else:
                tdi += timeshift(eta_dict[delay_array[i]['measurement']], total_delay_time * self.fsample, order=self.order)
        return tdi
    ...
    def CalculateNestedDelay(self, Delay_string, measurement, doppler=True):
        ...
        for i in range(N_delay):
            delay_idx = delays[i]
            total_delay_time = total_delay_time - timeshift(self.delays[delay_idx], total_delay_time * self.fsample, order=self.delay_order)
        if doppler:
            total_delay_doppler = np.gradient(total_delay_time, 1./self.fsample)
            return (1. + total_delay_doppler) * timeshift(measurement, total_delay_time * self.fsample, order=self.order)
        else:
    ...
    def CalculateNestedDelayCombination(self, Delay_string_plus, Delay_string_minus, measurement, doppler=True):
        ...
        for i in range(N_plus):
            plus_delay += self.CalculateNestedDelay(Delay_string=Delay_string_plus[i], measurement=measurement, doppler=doppler)
        for i in range(N_minus):
            minus_delay += self.CalculateNestedDelay(Delay_string=Delay_string_minus[i], measurement=measurement, doppler=doppler)
    ...
    def CalculateBasicTDI(self, channel='X2', doppler=True, channel_name='tdi_channel'):
        ...
        if isinstance(channel, str):
            ...
            if channel == 'X1':
                plus_paths = [self.X1_plus, ]
                minus_paths = [self.X1_minus, ]
            elif channel == 'X2':
                plus_paths = [self.X2_plus, ]
                minus_paths = [self.X2_minus, ]
            elif channel == 'alpha1':
                plus_paths = [self.alpha1_plus, ]
                minus_paths = [self.alpha1_minus, ]
            elif channel == 'alpha2':
                plus_paths = [self.alpha2_plus, ]
                minus_paths = [self.alpha2_minus, ]
            else:
                raise NotImplementedError('channel ' + channel + ' not implemented.')
        elif isinstance(channel, list):
            plus_paths = [channel[0], ]
            minus_paths = [channel[1], ]
        else:
            raise ValueError('channel should be string or list.')

        self.measurements[channel_name] = self.CalculatePathCombination(plus_paths=plus_paths, minus_paths=minus_paths, eta_dict=self.measurements['eta'], doppler=doppler)
        logger.info('TDI channel ' + channel_name + ' calculated.')
    
    def CalculateClockTDI(self, channel='X2', doppler=True, modulation_correction=True, channel_name='tdi_channel'):
        """ 
        ...
        if modulation_correction:
            ...
            for i, j, k in ['123', '231', '312']:
                dM[i] = (self.measurements['ref_sb'][i+k] - self.measurements['ref_c'][i+k] \
                         - self.measurements['ref_sb'][i+j] + self.measurements['ref_c'][i+j]) / 2.
            for i, j, k in ['123', '231', '312']:
                delayed_dM = timeshift(
                    dM[j], 
                    -self.fsample * self.delays[i+j],
                    order=self.order
                )
                if doppler:
                    delayed_dM *= (1. - self.measurements['dpl'][i+j])
        ...
        for i, j, k in ['123', '231', '312']:
            clock_correction += (self.measurements['b'][j+k] - self.measurements['a'][i+j]) * R[i+j] \
            - (self.measurements['b'][i+j] + self.measurements['a'][i+k]) * R[i+k] \
            + self.measurements['b'][j+k] * Pr[i+j]
    ...
    def CalculateFullTDI(self, channel='X2', channel_name = 'tdi_channel', doppler=True, clock_correction=True, garbage_time=None):
        if isinstance(channel, str):
            channel_name = channel
    ...
    def FastMichelson(self, doppler=True, channel='X'):
        ...
        if channel in ['X', 'XYZ', 'AET']:
            X0plus = self.CalculateNestedDelay(Delay_string='12', measurement=self.measurements['eta']['21'], doppler=doppler) + self.measurements['eta']['12']
            X0minus = self.CalculateNestedDelay(Delay_string='13', measurement=self.measurements['eta']['31'], doppler=doppler) + self.measurements['eta']['13']
            X0 = X0plus - X0minus
            ...
            if channel in ['XYZ', 'AET']:
                X0plus = self.CalculateNestedDelay(Delay_string='23', measurement=self.measurements['eta']['32'], doppler=doppler) + self.measurements['eta']['23']
                X0minus = self.CalculateNestedDelay(Delay_string='21', measurement=self.measurements['eta']['12'], doppler=doppler) + self.measurements['eta']['21']
                Y0 = X0plus - X0minus
                ...
                X0plus = self.CalculateNestedDelay(Delay_string='31', measurement=self.measurements['eta']['13'], doppler=doppler) + self.measurements['eta']['31']
                X0minus = self.CalculateNestedDelay(Delay_string='32', measurement=self.measurements['eta']['23'], doppler=doppler) + self.measurements['eta']['32']
                Z0 = X0plus - X0minus
    ...
    def FastMichelsonGen1(self, doppler=True, channel='X'):
        """  
            calculate the 1st-generation Michelson TDI channels with an optimized algorithm
        """
        if channel in ['X', 'XYZ', 'AET']:
            X0plus = self.CalculateNestedDelay(Delay_string='12', measurement=self.measurements['eta']['21'], doppler=doppler) + self.measurements['eta']['12']
            X0minus = self.CalculateNestedDelay(Delay_string='13', measurement=self.measurements['eta']['31'], doppler=doppler) + self.measurements['eta']['13']
            ...
            for (prefactor, DStrings) in PStrings[key]: 
                ...
                for DString in DStrings[::-1]:
                    ...
                    if doppler == True: 
                        ...
                        delayed_eta_ij = timeshift(data=delayed_eta_ij, shifts=delay_sign*self.fsample*tmp_delay, order=self.order) * (1. - tmp_doppler)
                    else: 
                        delayed_eta_ij = timeshift(data=delayed_eta_ij, shifts=delay_sign*self.fsample*delays[DString[-2]+DString[-1]], order=self.order)
...
def RangingProcessing(m, modulation_correction=True, doppler=True, order=31, drop1=0, drop2=0):
    ...
    dpl = {}
    for i, j, k in ['123', '231', '312']:
        dpl[i+j] = (Delta[i+j] + 1.e6) / modulation_freqs[i+k]
        dpl[i+k] = (Delta[i+k] - 1.e6) / modulation_freqs[i+j]
    ...
    if modulation_correction == True:
        dM = {}
        delta = measurements['ref_sb'] - measurements['ref_c']
        for i, j, k in ['123', '231', '312']:
            dM[i] = (delta[i+k] + 1.e6) / 2. - (delta[i+j] - 1.e6) / 2.
        dM = SCDict(dM)
        for i, j, k in ['123', '231', '312']:
            delayed_dM = timeshift(
                dM[j], 
            ...
            if doppler:
                delayed_dM *= (1. - measurements['dpl'][i+j])
    ...
    for key in ['mpr', 'ppr', 'dpl', 'sci_c', 'sci_sb', 'ref_c', 'ref_sb', 'tm_c', 'time']:
        measurements[key] = measurements[key].drop_edge_points(drop1, drop2)
    return measurements

def TheoreticalGWTDI(y, channel='X2', fsample=4, T=10., order=5):
    """
    y = Interferometer.gw
    ...
    if channel == 'X2':
        ...
        tmp3 = tmp1 + timeshift(tmp2, -fsample * 2. * T, order)
        tmp4 = tmp2 + timeshift(tmp1, -fsample * 2. * T, order)
        Xplus = tmp3 - timeshift(tmp3, -fsample * 4. * T, order)
        Xminus = tmp4 - timeshift(tmp4, -fsample * 4. * T, order)
        return Xplus - Xminus
    elif channel == 'Y2':
        ...
        tmp3 = tmp1 + timeshift(tmp2, -fsample * 2. * T, order)
        tmp4 = tmp2 + timeshift(tmp1, -fsample * 2. * T, order)
        Yplus = tmp3 - timeshift(tmp3, -fsample * 4. * T, order)
        Yminus = tmp4 - timeshift(tmp4, -fsample * 4. * T, order)
        return Yplus - Yminus
    elif channel == 'Z2':
        ...
        tmp3 = tmp1 + timeshift(tmp2, -fsample * 2. * T, order)
        tmp4 = tmp2 + timeshift(tmp1, -fsample * 2. * T, order)
        Zplus = tmp3 - timeshift(tmp3, -fsample * 4. * T, order)
        Zminus = tmp4 - timeshift(tmp4, -fsample * 4. * T, order)
        return Zplus - Zminus
    else:
...
class TDISensitivity():
    ...
    def __init__(self, Ri, nij, dij, L=L_nominal, S_OMS=SOMS_nominal, S_ACC=SACC_nominal):
        ...
        R0 = (self.Ri["1"] + self.Ri["2"] + self.Ri["3"]) / 3. # (3,)
        self.qi = dict()
    ...
    def polar_basis(self, lam, beta): 
        ...
        """
        u = np.array([np.sin(lam), -np.cos(lam), 0.])
    ...
    def Sinc_ij(self, k, f):
        sinc_ij = dict() 
        for key in MOSA_labels:
            sinc_ij[key] = np.sinc(f * self.dij[key] * (1. - np.dot(k, self.nij[key]))) # (Nf,)
