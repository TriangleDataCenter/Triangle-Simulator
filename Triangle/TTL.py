...
class TTL():
    ...
    def PSD_angular_jitters(self, f, asd, unit='angular_velocity'):
        if unit == 'rad':
            return asd ** 2 * (1. + (8e-4 / f) ** 4)
        elif unit == 'angular_velocity':
            return asd ** 2 * (1. + (8e-4 / f) ** 4) * (2. * np.pi * f) ** 2
        else:
    ...
    def PSD_DWS_readouts(self, f, asd, unit='angular_velocity'): # the same shape as isi readout noise
        if unit == 'rad':
            return asd ** 2 * (1. + (2e-3 / f) ** 4)
        elif unit == 'angular_velocity':
            return asd ** 2 * (1. + (2e-3 / f) ** 4) * (2. * np.pi * f) ** 2
        else:
            raise NotImplementedError('unit not implemented.')
    ...
    def SimulateAngularJitters(self):
        ...
        for key in ['12', '23', '31']:
            self.eta_jitters[key] = SC_eta_jitters[key[0]] * np.cos(np.pi / 6.) - SC_theta_jitters[key[0]] * np.sin(np.pi / 6.) + MOSA_eta_jitters[key]
        for key in ['21', '32', '13']:
            self.eta_jitters[key] = SC_eta_jitters[key[0]] * np.cos(np.pi / 6.) + SC_theta_jitters[key[0]] * np.sin(np.pi / 6.) + MOSA_eta_jitters[key]
