...
class OffsetFunctions():
    ...
    def ClockOffsets(self, t0=default_clock_t0, y0=default_clock_y0, y1=default_clock_y1, y2=default_clock_y2):
        """
        clock offsets [s]
        """
        flag = self.clock_flag
        ...
        if flag:
            for label in SC_labels:  
                func[label] = np.poly1d([1. / 3. * y2[label], 0.5 * y1[label], y0[label], t0[label]])   
        else:
            for label in SC_labels:
                func[label] = self._zero_function
        return func    
