...
def SSBTimetoDetectorTime(time_ssb, orbit, longitude, latitude):
    wave_vector = -np.array([np.cos(longitude) * np.cos(latitude), np.sin(longitude) * np.cos(latitude), np.sin(latitude)])
...
class GW():
    ...
    def CalculateResponse(self, time_dict):
        ...
        for label in MOSA_labels:
            ...
            Denominator = (np.dot(wave_vector, (arm_data[label]).T) - 1.) * (-2.)  # (N) array
...
class MBHB():
    ...
    def __call__(self, Mc, q, spin1z, spin2z, tc, phic, D, inc, dt=10., f_lower=None, mass_scale=None):
        """ 
        Args: 
            Mc in [MSUN]
        ...
        # set f_lower 
        Tobs = tc / YEAR
        if f_lower == None: # calculate the lower limit of frequency to the leading order
        ...
        hp, hc = wf.get_td_waveform_from_fd(
            ...
            delta_t = dt / mass_scale, f_lower = f_lower, 
