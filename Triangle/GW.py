...
class General_Injection():
    ...
    def __call__(self, params, times):
        ...
        return t_data, hSp_data + 1.j * hSc_data 
...
class MBHB_FastLISA():
    ...
    def __call__(self, Mc, q, spin1z, spin2z, tc, phic, D, inc, psi, T=1., dt=10.):
        """ 
        Args: 
        ...
        if self.buffer: # the buffered f_lower is at least 1.5 times lower 
            f_lower = min(f_lower * 2. / m_max, f_lower / 1.5) 
        f_lower = max(f_lower, 1e-5)  # set a lower limit of 1e-5 Hz
        f_lower *= mass_scale # get rescaled lower frequency
        if self.verbose == 1: 
        ...
        hp, hc = wf.get_td_waveform(
            ...
            coa_phase = phic,
            distance = D, inclination = inc, 
            delta_t = dt_wf / mass_scale, f_lower = f_lower, 
            mode_array = self.modes
            )
...
class MBHB_v5_Injection():
    ...
    def __call__(self, params, times):
        ...
        s2z = spin2z
        f_max = 1024.0 # Hz 
        f_min = 0.0157 / (Mt * np.pi * lal.MTSUN_SI) / 10. # Hz 
        deltaF = 0.125
        params_dict = {
            "mass1": m1,
            "mass2": m2,
        ...
        self.sampling_rate = 1. / (self.t_data[1] - self.t_data[0])
        ...
        return self.t_data, self.hp_data + 1.j * self.hc_data    
        ...
        if self.linear_interp:
            for ikey, key in enumerate(MOSA_labels):
                if self.Ndelay_dict[key] == 0:
                    ...
                    Denominator = (self.xp.matmul(self.arm_vector_dict[key], wave_vector) - 1.) * (-2.)  # (Ntime)
                    ...
                    for Idelay in range(self.Ndelay_dict[key]): 
                        ...
                        hphc_send = self.xp.interp(x=t_send, xp=times_interp, fp=hphc0, left=0., right=0.) # each (Ntime)
                        hphc_recv = self.xp.interp(x=t_recv, xp=times_interp, fp=hphc0, left=0., right=0.) # each (Ntime)
