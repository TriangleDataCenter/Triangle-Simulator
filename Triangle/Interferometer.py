...
#         self.tm_sb_ifo = dict(offset=None, fluctuation=None, total=None)
    ...
    def SimulateProperTimes(self):
        ...
        self.tcb_time = assign_function_for_SCs(
            functions=self.orbit_class.TCBinTPSfunctions,
    ...
    def SimulateProperRanges(self):
        ...
        self.ppr = assign_function_for_MOSAs(
            functions=self.orbit_class.PPRfunctions(),
            proper_time=self.proper_time) 
        self.dppr = assign_function_for_MOSAs(
            functions=self.orbit_class.DPPRfunctions(),
            proper_time=self.proper_time)
    ...
    def SimulateBasicNoise(self):
        ...
        if self.laser_noise_flag:
            self.BasicNoise['laser_noise'] = assign_noise_for_MOSAs(
                self.noise_class.PSD_LASER,
                fsample=self.fsample,
                size=self.size)
            self.BasicNoise['laser_noise'] *= F_LASER # convert to freq

        if self.ob_noise_flag:
            self.BasicNoise['ob_noise'] = assign_noise_for_MOSAs(
                self.noise_class.PSD_OB,
                fsample=self.fsample,
                size=self.size)
            
        if self.clock_noise_flag:
            self.BasicNoise['clock_noise'] = assign_noise_for_SCs(
                self.noise_class.PSD_CLOCK,
                fsample=self.fsample,
                size=self.size)
            
        if self.bl_noise_flag:
            self.BasicNoise['bl_noise'] = assign_noise_for_MOSAs(
                self.noise_class.PSD_BL,
                fsample=self.fsample,
                size=self.size)   
            
        if self.ranging_noise_flag:
            R_in_s = lambda f: self.noise_class.PSD_R(f) / (2. * np.pi * f) ** 2
            self.BasicNoise['ranging_noise'] = assign_noise_for_MOSAs(
                R_in_s,
                fsample=self.fsample,
                size=self.size)  
            
        if self.op_noise_flag:
            if SOP_OTHER_nominal == 0.:
    ...
    def CleanBasicNoise(self, noise_type):
        if not noise_type in self.BasicNoise.keys():
    ...
    def SimulateGW(self):
        ...
        if isinstance(self.gw_class, list):
            ...
            if self.pool == None:
                self.gw = assign_const_for_MOSAs(0.)
                for i in range(NGW):
    ...
    def SimulateClocks(self):
        ...
        # clock offsets
        self.clock_offset = assign_function_for_SCs(
            functions=self.offset_class.ClockOffsets(),
            proper_time=self.proper_time)
        self.clock_freq_offset = assign_function_for_SCs(
            functions=self.offset_class.ClockFreqOffsets(),
            proper_time=self.proper_time)
    ...
    def SimulateSources(self):
        ...
        # 1. offsets
        self.laser_offset = assign_function_for_MOSAs(
            functions=self.offset_class.LaserOffsets(),
            proper_time=self.proper_time)
        # 2. fluctuations
        # already generated.
        ...
        if self.fplan_flag:
            ...
            for key in lock_order:
                if lock_topology[key] == 'primary':
                    ...
                    self.laser_offset[key] = self.laser_offset[key_ad] - self.fplan[key] * (1. + self.clock_freq_offset[key[0]])
                    # 2. fluctuation
                    ...
                    # 1. offset
                    key_ds = key[1] + key[0]
                    delayed_distant = timeshift(self.laser_offset[key_ds], -self.ppr[key] * self.fsample, self.order)
                    doppler_delayed_distant = delayed_distant * (1. - self.dppr[key])    
                    self.laser_offset[key] = doppler_delayed_distant \
                    - F_LASER * self.dppr[key] \
                    - self.fplan[key] * (1. + self.clock_freq_offset[key[0]])
                    # 2. fluctuation
                    delayed_distant_ln = timeshift(self.BasicNoise['laser_noise'][key_ds], -self.ppr[key] * self.fsample, self.order)
                    doppler_delayed_distant_ln = delayed_distant_ln * (1. - self.dppr[key])  
                    self.BasicNoise['laser_noise'][key] += doppler_delayed_distant_ln 
                    if self.clock_noise_flag:
                        self.BasicNoise['laser_noise'][key] += -self.fplan[key] * self.BasicNoise['clock_noise'][key[0]]
                    if self.ob_noise_flag:
                        delayed_distant_on = timeshift(self.BasicNoise['ob_noise'][key_ds], -self.ppr[key] * self.fsample, self.order)
                        doppler_delayed_distant_on = delayed_distant_on * (1. - self.dppr[key])  
                        self.BasicNoise['laser_noise'][key] += (F_LASER + delayed_distant) \
                        * (doppler_delayed_distant_on + self.BasicNoise['ob_noise'][key])
                    if self.op_noise_flag:
        ...
        # side band 
        self.laser_offset_sb = self.laser_offset + self.modulation_freqs * (1. + self.clock_freq_offset.toMOSA())
    ...
    def SimulateOffsets(self):
        ...
        # ref_sb
        self.ref_sb_ifo['offset'] = self.ref_c_ifo['offset'] \
        + (self.modulation_freqs.adjacent() - self.modulation_freqs) * (1. + self.clock_freq_offset.toMOSA())
        # tm_c
    ...
    def SimulateFluctuations(self):
        ...
        # tm noise 
        if self.acc_noise_flag:
            self.sci_c_ifo['fluctuation'] += 0.
            self.sci_sb_ifo['fluctuation'] += 0.
            self.ref_c_ifo['fluctuation'] += 0.
            self.ref_sb_ifo['fluctuation'] += 0.
            self.tm_c_ifo['fluctuation'] += (F_LASER + self.laser_offset) * (-2.) * self.BasicNoise['acc_noise']
        ...
        if self.ob_noise_flag:
            ...
            self.sci_c_ifo['fluctuation'] += (F_LASER + (self.laser_offset.reverse()).timedelay(**d_args, pool=self.pool)) \
            * displacement
            self.sci_sb_ifo['fluctuation'] += (F_LASER + (self.laser_offset_sb.reverse()).timedelay(**d_args, pool=self.pool)) \
            * displacement
            self.ref_c_ifo['fluctuation'] += 0.
            self.ref_sb_ifo['fluctuation'] += 0.
            self.tm_c_ifo['fluctuation'] += (F_LASER + self.laser_offset) * 2. * self.BasicNoise['ob_noise']
            displacement = None
    ...
    def SimulateTimeShift(self):
        ...
        clock_factor = (1. / (1. + self.total_clock_freq_deviation)).toMOSA()
        clock_offset_factor = (1. / (1. + self.clock_freq_offset)).toMOSA()
        ...
        # 1) add clock noise
        if self.clock_noise_flag:
            cfactor = self.BasicNoise['clock_noise'].toMOSA() / (1. + self.clock_freq_offset.toMOSA())
