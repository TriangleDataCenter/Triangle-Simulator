...
from numpy.fft import fftfreq, fft, rfft, rfftfreq
import numpy as np
...
def FFT_window(data_array, fsample, window_type = None, window_args_dict = window_dict_default):
    ...
    f = rfftfreq(n_data, 1. / fsample)[1: n_f]
...
def PSD_window(data_array, fsample, nbin = 1, window_type = None, window_args_dict = window_dict_default, detrend='constant'):
    """  
    ...
    f, Sf = welch(x=data_array, fs=fsample, window=window_array, return_onesided=True, nperseg=nperseg, detrend=detrend)
    return f[1:], Sf[1:]



def CSD_window(data_array1, data_array2, fsample, nbin = 1, window_type = None, window_args_dict = window_dict_default, detrend='constant'):
    """  
    ...
    if window_type == None:
        ...
        if window_type == 'tukey':
            ...
            raise ValueError('Unsupported window type.')
        window_array = window(M=nperseg, **window_args_dict)
    f, Sf = csd(x=data_array1, y=data_array2, fs=fsample, window=window_array, return_onesided=True, nperseg=nperseg, detrend=detrend)
    return f[1:], Sf[1:]


def plot_ASDs(data_arrays, labels, fsample, nbin = 1, window_type = None, window_args_dict = window_dict_default, psd_funcs = None, file = None):
